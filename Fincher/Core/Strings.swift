import Foundation
class Strings {
    static let shared = Strings()
    let calcOptionString = NSLocalizedString(
        "Calculation option", bundle: Bundle.main, comment: "Calculation option")
    let calcMonthlyPaymentString = NSLocalizedString(
        "Calculation of monthly payment", bundle: Bundle.main, comment: "Calculation of monthly payment")
    let calcLoanTermString = NSLocalizedString(
        "Loan term calculation", bundle: Bundle.main, comment: "Loan term calculation")
    let calcMlaString = NSLocalizedString(
        "Calculation of the mla", bundle: Bundle.main, comment: "Calculation of the mla")
    let amountOfCreditString = NSLocalizedString(
        "Amount of credit", bundle: Bundle.main, comment: "Amount of credit")
    let calcOptionsTipString = NSLocalizedString(
        "calcOptionTip", bundle: Bundle.main, comment: "Calculation Options Tip")
    let okString = NSLocalizedString(
        "Ok, i got it", bundle: Bundle.main, comment: "Ok, i got it")
    let yearString = NSLocalizedString(
        "year", bundle: Bundle.main, comment: "year")
    let monthString = NSLocalizedString(
        "month", bundle: Bundle.main, comment: "month")
    let creditTermString = NSLocalizedString(
        "Credit Term", bundle: Bundle.main, comment: "Credit Term")
    let interestRateString = NSLocalizedString(
        "Interest Rate", bundle: Bundle.main, comment: "Interest Rate")
    let monthlyPaymentString = NSLocalizedString(
        "Monthly Payment", bundle: Bundle.main, comment: "Monthly Payment")
    let annuitentTipString = NSLocalizedString(
        "Annuitent Payment", bundle: Bundle.main, comment: "Annuitent Payment")
    let diffTipString = NSLocalizedString(
        "Diff Payment", bundle: Bundle.main, comment: "Diff Payment")
    let annuityString = NSLocalizedString(
        "annuity", bundle: Bundle.main, comment: "annuity")
    let diffString = NSLocalizedString(
        "differentiated", bundle: Bundle.main, comment: "differentiated")
}
